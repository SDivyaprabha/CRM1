...
namespace CRM
{
    public partial class frmServiceOrderBill : Form
    {
        ...
        private void frmServiceOrderBill_FormClosed(object sender, FormClosedEventArgs e)
        {
            if (m_iRegBillId != 0) BsfGlobal.ClearUserUsage("CRM-ServiceBill-Edit", m_iRegBillId, BsfGlobal.g_sCRMDBName);
            ...
            if (BsfGlobal.g_bWorkFlow == true)
            {
                if (m_iRegBillId != 0)
                {
                    ...
                    ChangeGridValue(m_iRegBillId);
                    frmServiceOrderBillReg.m_oDW.Show();
                    frmServiceOrderBillReg.m_oDW.Select();
        ...
        private void ClearEntries()
        {
            ...
            if (cboQuote.Enabled == false)
            {
                cboQuote.Enabled = true;
            }
            cboProject.ItemIndex = 0;
        ...
        private void compAddBtn_ItemClick(object sender, DevExpress.XtraBars.ItemClickEventArgs e)
        {
            ...
            if (Convert.ToInt32(cboQuote.EditValue) == -1 || Convert.ToInt32(cboQuote.EditValue) == 0)
            {
                MessageBox.Show("Select Quote", "ASSET", MessageBoxButtons.OK, MessageBoxIcon.Asterisk);
                cboQuote.Focus();
                return;
            }
            m_sSerId = "";
